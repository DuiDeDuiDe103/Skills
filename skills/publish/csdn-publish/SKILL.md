...
2. **navigate 返回 404 时不得中断流程**。CSDN 是 SPA 路由，navigate 工具返回的 HTTP 状态码 / Title 不可靠。每次 navigate 到发布页后，必须用 `javascript_tool` 执行以下校验：
   ```js
   document.title.includes('写文章')
   ```
   返回 `true` 则继续下一步；返回 `false` 或 `undefined` → 等待 2s 后重试一次，仍失败再报告用户。
   > **javascript_tool 返回值注意**：该工具对复杂表达式（如 `JSON.stringify({...})`）可能返回 `undefined`，即使代码实际执行成功。应尽量用简单表达式（字符串、布尔值、数字）作为返回值。需要结构化输出时，用字符串拼接代替 `JSON.stringify`。
...
> document.title.includes('写文章')
> ```
> 返回 `true` → 继续。返回 `false` → 等 2s 重试一次，仍失败再报告用户。
...
**首选方案**：用 `read_page`（filter=interactive）找到标题输入框的 ref ID，再用 `form_input` 赋值：

```
read_page(filter="interactive") → 找到 textbox "请输入文章标题..." 的 ref（如 ref_9）
form_input(ref="ref_9", value="文章标题")
```

> **不要用 `Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set`**：CSDN 页面上该方法会抛 `Illegal invocation` 错误。直接 `inp.value = x` 也不会触发 Vue 响应。`form_input` 工具内部已处理了这些兼容性问题。
...
1. 用 `read_page`（filter=interactive）找到「添加文章标签」按钮的 ref（如 ref_55），然后用 `computer` 工具点击：
   ```
   read_page(filter="interactive") → 找到 button "添加文章标签" 的 ref
   computer(action="left_click", ref="ref_55")
   ```
   > **不要用 JS `.click()`**：CSDN 的 Vue 按钮对 `btn.click()` 经常返回 `undefined` 且不触发实际事件。`computer` 工具模拟真实鼠标点击，可靠性更高。

2. **等待标签弹窗出现**（1 秒后轮询）：
   ```js
   // 等 1s 后执行
   const inp = document.querySelector('input[placeholder*="请输入文字搜索"]');
   inp !== null && inp.offsetParent !== null
   ```
   返回 `true` → 弹窗已出现，继续。返回 `false` → 再等 1s 重试，最多 3 次。

3. **推荐标签**：用 JS 遍历 `[role="tabpanel"]` 里的 `span`，按文本**精确匹配**后 `.click()`。**每个标签名匹配到第一个就 `break`，避免重复点击导致 toggle 取消**：
   ```js
   // 示例：在 Java panel 中选 spring
   const panels = document.querySelectorAll('[role="tabpanel"]');
   const spans = panels[2].querySelectorAll('span');
   for (const s of spans) {
     if (s.textContent.trim() === 'spring') { s.click(); break; }
   }
   ```
   - Java 相关 → `panels[2]`（tab「Java」对应 panel）
   - 后端相关 → `panels[8]`（tab「后端」）
   - 面试 / 职场 → `panels[29]`（tab「学习和成长」）
   > **切换 panel 后等 500ms** 再操作下一个 panel，避免面板未渲染。
...
5. 校验：
   ```js
   document.querySelectorAll('.el_mcm-tag.is-closable').length
   ```
   应 ≥ 期望标签数。

### 5. 保存草稿

1. 用 `read_page`（filter=interactive）找到「保存草稿」按钮的 ref（如 ref_68），然后用 `computer` 工具点击：
   ```
   computer(action="scroll_to", ref="ref_68")  // 先滚到可见
   computer(action="left_click", ref="ref_68")
   ```
   > **不要用 JS `btn.click()`**：CSDN 的保存按钮是 Vue 绑定的，JS `.click()` 返回 `undefined` 且不触发保存。必须用 `computer` 工具模拟真实点击。

2. 等待 3-4 秒后验证保存是否成功。**双重验证策略**：

   **验证方式 A — el-message**（可能已消失，不一定能捕获）：
   ```js
   document.querySelector('.el-message')?.textContent?.includes('草稿保存成功')
   ```

   **验证方式 B — 网络请求**（更可靠，作为兜底）：
   ```
   read_network_requests(urlPattern="saveArticle")
   ```
   检查是否存在 `POST 200 ... saveArticle` 记录。有则保存成功。
...
- **CSDN 按钮优先用 `computer` 点击**：CSDN 是 Vue SPA，JS `btn.click()` 对很多按钮（保存草稿、添加文章标签等）返回 `undefined` 且不触发实际事件。`computer(action="left_click", ref=...)` 模拟真实鼠标点击，可靠性远高于 JS click。
- **标题输入用 `form_input`**：`Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set` 在 CSDN 页面抛 `Illegal invocation`；直接 `inp.value = x` 不触发 Vue 响应。`form_input` 工具内部已处理兼容性。
- **标签选择必须 `break`**：`querySelectorAll('span')` 遍历时，同一个标签文本可能被多个嵌套 span 包含。不 `break` 会导致同一个标签被点击两次（toggle 效果 = 没选上）。
- **标签弹窗需要显式等待**：点击「添加文章标签」后弹窗不是瞬间出现的，必须等 1s 后轮询 `input[placeholder*="请输入文字搜索"]` 的可见性。
- **`.el-message` 不可靠**：CSDN 的保存成功提示可能瞬间出现又消失，`javascript_tool` 轮询时经常捕获不到。**兜底方案**：用 `read_network_requests(urlPattern="saveArticle")` 检查是否有 `POST 200` 记录，这比 DOM 检测更可靠。
- **`javascript_tool` 返回值陷阱**：`JSON.stringify({...})` 等复杂表达式在该工具中可能返回 `undefined`，即使代码实际执行成功。应尽量用简单表达式（布尔值、数字、字符串）作为返回值。需要结构化输出时用字符串拼接代替。
...
Agent：
  ...
  4. CORS server 传数据 → fetch 解码 → marked.parse → setData(html)
  5. read_page → form_input 设标题
  6. computer 点击「添加文章标签」→ 等弹窗 → panels 选 spring、jvm、后端、面试
  7. computer 点击「保存草稿」→ read_network_requests 确认 saveArticle POST 200
  8. 关 CORS server → 回执：标题 / 标签 / 草稿箱链接
```
...
- [ ] 保存成功确认（二选一）：
  ...
  - `read_network_requests(urlPattern="saveArticle")` 存在 `POST 200` 记录
- [ ] CORS server 已关闭
- [ ] 给用户回执包含草稿箱 URL
